...
class ISPRSDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(512, 512),
        ...
        for name in self.img_ids:
            ...
            #img_file = osp.join(self.root, "train/merge/%s.tif" % name)
            img_file = osp.join(self.root, "train/merge3/%s.tif" % name)
...
class ISPRSDataValSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #self.mean=[np.mean(image[:,:,0:1]),np.mean(image[:,:,1:2]),np.mean(image[:,:,2:3]), np.mean(image[:,:,3:4])]
...
class ISPRSDataTestSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, mean=(128, 128, 128), scale=False, mirror=True, ignore_label=255):
        ...
        for name in self.img_ids:
            ...
            #img_file = osp.join(self.root, "real_val/merge/%s.tif" % name)
            img_file = osp.join(self.root, "track2_test_data/merge/%s.tif" % name)
