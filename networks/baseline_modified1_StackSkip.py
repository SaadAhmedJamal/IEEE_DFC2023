...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        #self.conv0 = conv3x3(4, 64, stride=2)   
        ...
        self.conv1x1 = nn.Conv2d(3584, 2048, kernel_size=1)
        #self.conv128x64 = nn.Conv2d(128, 64, kernel_size=1)
        #self.conv512x256 = nn.Conv2d(512, 256, kernel_size=1)


        #self.conv_transpose1 = nn.ConvTranspose2d(256, 256, kernel_size=2, stride=2, padding=0, output_padding=0)
        ...
        #elf.layer5 = self._make_layer(block, 512, layers[3], stride=1, dilation=4, multi_grid=(1,1,1))
        ...
        self.head = nn.Sequential(nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
                                  ...
                                  #nn.ConvTranspose2d(1, 1, kernel_size=1, stride=2, padding=0, bias=True),
                                  nn.Sigmoid())

        #self.ppm = ASPPModule(3584, 256)

        self.ppm = PSPModule(2048, 256)
        #self.asp = ASPPModule(2048, 256)
        #self.ppm = PSPModule(3584, 256)
        #self.ppm = PSPModule(3840, 256)
    ...
    def forward(self, x):
        ...
        #x0 = self.conv128x64(x0) # (2048, 64, 64)

        x1 = self.layer1(x0)  # (256, 128, 128)
        x2 = self.layer2(x1) # (512, 64, 64)
        x3 = self.layer3(x2) # (1024, 64, 64) 3584
        ...
        #x1_skip = torch.cat((x4,x4),dim=1)
        #x1_skip = torch.cat((x3,x3),dim=1)
        #x1_skip = self.avgpool(x1_skip) # (2048, 64, 64)
        #x1_skip = self.conv1x1(x1_skip) # (2048, 64, 64)
        #print(x1_skip)
        ...
        #asp_out = self.asp(x1_skip)

        f = psp_out
        #f = self.conv512x256(asp_out) 

        #f = self.conv_transpose1(f) 
        x = self.head(f)
