...
from dataset.dataset_modified1 import ISPRSDataSet, ISPRSDataValSet
...
IMG_MEAN = np.array((120.47595769, 81.79931481, 81.19268267, 0.1019934569), dtype=np.float32)
...
#RESTORE_FROM = r'C:\Users\saada\Desktop\Drive\UBS\Workshop_Contest\track2\HeightEstimation\workspace\pretrain\my_resnet.pth'
#RESTORE_FROM = r'C:\Users\saada\Desktop\Drive\UBS\Workshop_Contest\track2\HeightEstimation\workspace\pretrain\my_resnet_model_101.pth'
RESTORE_FROM = r'C:\Users\saada\Desktop\Drive\UBS\Workshop_Contest\track2\HeightEstimation\workspace\pretrain\my_resnet_model_101b.pth'
#RESTORE_FROM = r'C:\Users\saada\Desktop\Drive\UBS\Workshop_Contest\track2\HeightEstimation\workspace\pretrain\my_model_psp.pth'
...
#RESTORE_FROM = r'C:\Users\saada\Desktop\Drive\UBS\Workshop_Contest\track2\HeightEstimation\workspace\pretrain\resnet50-19c8e357.pth'
...
def main():
    ...
    #print("Printing load state dict")
    #print(net.load_state_dict(new_params))
    ...
    for i_iter, batch in enumerate(trainloader):
        ...
        #pred_up = F.upsample(input=preds[0], size=(labels.size(2), labels.size(2)), mode='bilinear', align_corners=True)
        #print(preds[0].shape)
        #pred_up = F.upsample(input=preds[0], size=(labels.size(2), labels.size(2)), mode='bilinear', align_corners=True)
        pred_up = F.upsample(input=preds[0], size=(labels.size(2), labels.size(2)), mode='bicubic', align_corners=True)

        #pred_up = F.upsample(input=preds[0], size=(labels.size(1), labels.size(2)), mode='bilinear', align_corners=True)
        #print(preds)
        #print(pred_up.shape)
        #print(pred_up)
        #pred_up = F.upsample(input=preds[0], size=(dsms.size(1), dsms.size(2)), mode='bilinear', align_corners=True)
        #pred_up = F.upsample(input=preds[0], size=(labels.size(1), labels.size(2)), mode='bilinear', align_corners=True)
        ...
        #r2 = r2_score(pred_up.cpu(), dsms.cpu())
        #r2 = r2score(pred_up.cpu(), dsms.cpu())
        print(r2)
        avgr2=avgr2+r2
        z=z+1
        if z % 980 == 0:
