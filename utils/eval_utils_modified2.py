...
def predict_sliding(net, image, tile_size, classes, flip_evaluation):
    ...
    for row in range(tile_rows):
        for col in range(tile_cols):
            ...
            img = image[:, 0:3, y1:y2, x1:x2]

            padded_img = pad_image(img, tile_size)
            
            sar_images = image[:, 3:4, y1:y2, x1:x2]
            padded_sar = pad_image(sar_images, tile_size)
            padded_sar = torch.from_numpy(padded_sar)
            padded_sar = torch.cat((padded_sar,padded_sar,padded_sar),dim =1)

            tile_counter += 1
            with torch.no_grad():
                padded_prediction = net(Variable(torch.from_numpy(padded_img)).cuda(),Variable(padded_sar).cuda())
